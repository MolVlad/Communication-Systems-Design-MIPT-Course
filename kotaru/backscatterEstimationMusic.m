function [Parameters, varargout] = backscatterEstimationMusic(csi_trace, M, N, c, fc, Ttot, fgap, ...
    SubCarrInd, d, paramRange, maxRapIters, useNoise, do_second_iter, runParam, varargin)
% INPUTS:
...
% в RAP затем используются только GridStart и GridSpacing
...
%% Applying MUSIC 
[Pn,Ps,Qn,Qs,EigenInfo] = GetQnBackscatter(X,EigDiffCutoff, nComps, runParam);
...
nIters = runParam.musicNumIter;
if paramRange.circularTx == 0

        for k=1:nIters
            % DelayStartStop = DelayStartStopOrig;%+(k-1)*2e-9; 
            % [delayFromMusicTmp,angleFromMusicTmp, maxCorr(k)]= SingleSnapshotRAPMusic(Qn,Qs,fc,fgap,d,K,L, ...
            % delayFromMusic,angleFromMusic,SubCarrInd,DelayStartStop);

            [delayFromMusicTmp,angleFromMusicTmp, deltaFromMusicTmp, maxCorr(k),music_spectrum] = ...
                RAPMusicGridMaxBackscatter(aTot,GridStart,GridSpacing,GridPts,Qn,Qs,fc,fgap,d,K,L, ...
                delayFromMusic,angleFromMusic, deltaFromMusic, SubCarrInd, deltaGridValue, ...
                u_sGridValue, delayGridValue, T, c, do_second_iter, seconditerGridPts, useNoise, k, paramRange, runParam);
            
            % дальше идет просто заполнение выходных полей
            if k==1
