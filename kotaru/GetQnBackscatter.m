...
function [Pn,Ps,Qn,Qs,EigenInfo] = GetQnBackscatter(X,EigDiffCutoff, nComps, runParam)

% % by singular value
% [U,D,~] = svd(X);
% % by eigenvalue
if runParam.printSizeX
    sizeX = size(X)
end
...
for k = 1:maxMultipath
    MDL(k) = -nSegments*(subarraySize-(k-1))*...
        log(geomean(lambdaTot(k:end))/mean(lambdaTot(k:end))) + 0.5*(k-1)*(2*subarraySize-k+1)*log(nSegments);
end
...
% SignalEndIdx определяет, сколько векторов относятся к шумовому
% пространству, а сколько к сигнальному. Это определяет матрица,
% передаваемая в main'е в backscatterEstimationMusic

if runParam.numSignalEigVector
    SignalEndIdx = runParam.numSignalEigVector;
end

if runParam.printNumSignalEigVector
    NumSignalEigVector = SignalEndIdx
end
